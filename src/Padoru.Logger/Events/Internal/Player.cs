...
namespace Padoru.Logger.Events.Internal
{
    public class Player : IEventHandler
    {
        ...
        {
            Plugin.Sender.AddToQuene(
                $"Игрок {player.Nickname}({player.UserId}) активировал процесс детонации Альфа-Боеголовки");
        }
        ...
        {
            Plugin.Sender.AddToQuene(
                $"Игрок {player.Nickname}({player.UserId}) остановил процесс детонации Альфа-Боеголовки");
        }
        ...
        {
            Plugin.Sender.AddToQuene(
                $"Игрок {player.Nickname}({player.UserId}) отключил питание в одной из зон комплекса");
        }
